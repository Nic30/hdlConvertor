...
from utils.antlr4._utils import collect_simple_rules, \
    remove_simple_rule, extract_option_as_rule, inline_rule, \
...
from utils.sv.syntax_fix import fix_SYSTEM_TF_IDENTIFIER, \
    ...
    fix_randomize_call, fix_dpi_import_export, fix_implications
...
def proto_grammar_to_g4(target_lang):
    ...
    add_eof(p.rules)
    numbers_add_whitespace_after_base(p.rules)
    Antlr4GenericOptimizer().optimize(p.rules)
    assignment_pattern__body_optional(p.rules)
