...
HdlStmProcess::HdlStmProcess() :
		iHdlStatement(), trigger_constrain(HdlStmProcessTriggerConstrain::NONE), body(
				create_object<HdlStmBlock>(nullptr)) {
}

HdlStmProcess::HdlStmProcess(
		unique_ptr<vector<unique_ptr<iHdlExprItem>>> _sensitivity) :
		iHdlStatement(), trigger_constrain(HdlStmProcessTriggerConstrain::NONE), sensitivity_list(
