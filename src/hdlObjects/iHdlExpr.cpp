...
iHdlExpr::iHdlExpr(iHdlExpr *op0, HdlOperatorType operatorType, iHdlExpr *op1) {
	assert(op0);
...
iHdlExpr* iHdlExpr::call(iHdlExpr *fnId, const std::vector<iHdlExpr*> &args) {
	iHdlExpr *e = new iHdlExpr();
	e->data = HdlCall::call(fnId, args);
	return e;
}
iHdlExpr* iHdlExpr::parametrization(iHdlExpr *fnId,
		const std::vector<iHdlExpr*> &args) {
	iHdlExpr *e = new iHdlExpr();
	e->data = HdlCall::parametrization(fnId, args);
	return e;
}
