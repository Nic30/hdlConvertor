parser grammar sv2017perfParser;
...
statement_item:
  ...
  | subroutine_call_statement #statement_itemSubroutine
  | disable_statement #statement_itemDissable
  | event_trigger     #statement_itemEv
  | loop_statement    #statement_itemLoop
  | jump_statement    #statement_itemJump
  | par_block         #statement_itemPar
  | procedural_timing_control_statement #statement_itemProcedOrTimeControl
  | seq_block                           #statement_itemSeq
  | wait_statement                      #statement_itemWait
  | procedural_assertion_statement      #statement_itemProcedAssert
  | randsequence_statement              #statement_itemRandSeq
  | randcase_statement                  #statement_itemRandCase
  | expect_property_statement           #statement_itemExpectProp
;
cycle_delay:
...
blocking_assignment:
 variable_lvalue ASSIGN delay_or_event_control expression
  | variable_lvalue ASSIGN dynamic_array_new
...
expression:
  ...
  | expression ( MUL
                  | DIV
                  | MOD
                  ) ( attribute_instance )* expression      #expressionMulDiv
  | expression ( PLUS
                 | MINUS
                 ) ( attribute_instance )* expression       #expressionAddSub
  | expression ( SHIFT_LEFT
                 | SHIFT_RIGHT
                 | ARITH_SHIFT_LEFT
                 | ARITH_SHIFT_RIGHT
                 ) ( attribute_instance )* expression       #expressionShift
  | expression ( LT
                 | LE
                 | GT
                 | GE
                 ) ( attribute_instance )* expression       #expressionCmp
  ...
  | expression ( EQ
                 ...
                 | WILDCARD_EQ
                 | WILDCARD_NEQ
                 ) ( attribute_instance )* expression       #expressionEq
  | expression AMPERSAND ( attribute_instance )* expression #expressionAnd
  | expression ( XOR
                    | NXOR
                    | XORN
                    ) ( attribute_instance )* expression    #expressionBitwise
  | expression  BAR ( attribute_instance )* expression      #expressionOr
  | expression  LOG_AND ( attribute_instance )* expression  #expressionLogAnd
  | expression  LOG_OR ( attribute_instance )* expression   #expressionLogOr
...
pattern:
 DOT ( MUL
  ...
  | APOSTROPHE_LBRACE ( pattern ( COMMA pattern )*
                      | identifier COLON pattern ( COMMA identifier COLON pattern )*
                      ) RBRACE
  | expression
;
assignment_pattern:
 APOSTROPHE_LBRACE ( expression ( COMMA expression )*
...
list_of_paths:
 identifier_doted_index_at_end ( COMMA identifier_doted_index_at_end )*;
...
ansi_port_declaration:
  ( port_direction ( net_or_var_data_type )?
   ...
   (ASSIGN constant_expression)? # ansiPortDeclarationNet
  | (port_direction)? DOT identifier_doted_index_at_end LPAREN (expression)? RPAREN       # ansiPortDeclarationIdentifier
;
