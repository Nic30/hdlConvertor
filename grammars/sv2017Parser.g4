...
concurrent_assertion_statement:
 ( KW_ASSERT 
      ...
      ( KW_DISABLE KW_IFF LPAREN expression_or_dist RPAREN )? sequence_expr 
...
bins_or_options:
 coverage_option 
      | ( ( KW_WILDCARD )? bins_keyword identifier ( ( LSQUARE_BR ( covergroup_expression )? RSQUARE_BR )? ASSIGN ( LBRACE covergroup_range_list RBRACE ( KW_WITH LPAREN covergroup_expression 
      RPAREN )? 
...
assignment_pattern:
 APOSTROPHE LBRACE ( expression ( COMMA expression )* 
                          | structure_pattern_key COLON expression ( COMMA structure_pattern_key COLON expression )* 
                          | array_pattern_key COLON expression ( COMMA array_pattern_key COLON expression )* 
